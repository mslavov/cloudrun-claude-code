      ...
      if (this.postExecutionActions.git && (this.postExecutionActions.git.commit || this.postExecutionActions.git.push)) {
        try {
          ...
          if (hasChanges && this.postExecutionActions.git.commit) {
            logger.info(`[TASK ${this.taskId}] Committing changes to git`);
            ...
            if (this.postExecutionActions.git.push) {
              logger.info(`[TASK ${this.taskId}] Pushing changes to remote`);
              ...
              logger.info(`[TASK ${this.taskId}] Changes pushed to ${branch}`);
            }
          } else if (!hasChanges) {
            logger.info(`[TASK ${this.taskId}] No changes to commit`);
          }
        } catch (gitError: any) {
